...
%在10kHz添加噪音信号
fz=10000;              %噪音频率
xz1=0.01*sin(2*pi*fz*t);%噪声序列，以sin为例
y1=y0+xz1';
sound(y1,fs);
...
% audiowrite('signal_after_noise_10khz.wav',real(y1),fs)
%%

b=[1 -exp(1i*2*pi*fz/fs)];
a=[0 1];
[h,f] = freqz(b,a,N/2);
...
fz1=[17500 0 0 0];              %噪音频率
xz1=1*(sin(2*pi*fz1(1)*t)+sin(2*pi*fz1(2)*t)+1*sin(2*pi*fz1(3)*t)+sin(2*pi*fz1(4)*t));%噪声序列，以sin为例
xz2=xz1';
y4=y0+xz1';
% sound(y4,fs);
% audiowrite('signal_after.wav',real(y4),fs)
...
fz2=Index-1%噪音频率
% xz1=1*(sin(2*pi*fz2*t));%噪声序列，以sin为例+sin(2*pi*fz1(2)*t)+sin(2*pi*fz1(3)*t)+sin(2*pi*fz1(4)*t)
%引入50Hz工频干扰
% d=sig_in+xz1';                  %引入特定频率干扰后的接收输入
%sound(real(d),fs)
%  ——造参考信号—————————————————————
x1=cos(2*pi*t*fz2(1));                 %参考输入1
x2=sin(2*pi*t*fz2(1));                 %参考输入2
%  ——Notch滤波—————————————————————
w1=0.5;
w2=0.5;                       %权矢量初值
e=zeros(1,N);
...
figure(12)
plot(t,e)
title('自适应滤波声音信号的时域波形图');xlabel('时间/s');ylabel('振幅');
sound(real(e),fs);

% audiowrite('signal_filter_noise_zsy_0.001.wav',real(e),fs)

%%
%自适应滤波
%多在几个频率加噪声
%添加其他频率点的啸叫,分别在1khz,8khz，20khz，30khz添加
% fz1=[1000 8000 20000 30000];              %噪音频率
